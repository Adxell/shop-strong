...
const payOrder = async(req: NextApiRequest, res: NextApiResponse<Data>) => {
    ...
    try {
        ...
        if ( !paypalBearerToken ) {
            return res.status(400).json({message: 'No se pudo hacer el pago en paypal'})
        }
        const { transactionId = '', orderId = ''} = req.body;
        const { data } = await axios.get<IPaypal.PaypalOrderStatus>(`${ process.env.PAYPAL_ORDERS_URL}/${transactionId}`, {
            headers: {
        ...
        if ( dbOrder.total !== Number(data.purchase_units[0].amount.value) ) {
            await db.disconnect()
            return res.status(400).json({message: 'Los monto de paypal y nuestra orden no son iguales'})
        }
